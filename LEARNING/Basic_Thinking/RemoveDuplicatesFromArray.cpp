...
// Here, we are modifying the given array itself.
// TC : O(n*n + n)
int * bruteForceMethod (int *inArray, int sizeOfArray)
{
...
// TC : O(nlogn + n + n)
int * sortAndRemoveDuplicates (int *inArray, int sizeOfArray)
{
...
// TC : O(n + n), SC : O(n)
int * useSetAndRemoveDup (int *inArray, int sizeOfArray)
{
    set<int> mySet;
    for (int i=0; i<sizeOfArray; ++i)
...
int main()
{
    ...
    int *outArray = useSetAndRemoveDup(myArray, n);
